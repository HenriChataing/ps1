...
static inline void DisassembleIType(Instruction *instr, char const *text,
    void (*fmt)(Instruction *, uint32_t, uint32_t, uint16_t)) {

    instr->Text = text;
    fmt(instr,
        n64::assembly::getRt(instr->RawBytes),
        n64::assembly::getRs(instr->RawBytes);
        n64::assembly::getImmediate(instr->RawBytes));
}

static void IType_Rt_Rs_Imm(Instruction *instr, uint32_t rt, uint32_t rs, uint16_t imm) {
    instr->Operands[0].text = RegisterNames[rt];
    instr->Operands[0].kind = InstructionOperandKind::Register;

    instr->Operands[1].text = RegisterNames[rs];
    instr->Operands[1].kind = InstructionOperandKind::Register;

    instr->Operands[2].text = fmt::format("{}", (int16_t)imm);
    instr->Operands[2].kind = InstructionOperandKind::Immediate;
}
...
static inline void DisassembleIType(Instruction *instr,
    void (*fmt)(Instruction *, uint32_t, uint32_t, uint16_t)) {
